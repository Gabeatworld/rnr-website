...
RNR.register('awardsFooterPin', function (/* shared */) {
  ...
  var buildTimeline = function () {
      ...
      scrollTrigger: {
        ...
        onUpdate: function (self) {
          ...
          if (p >= 0.50 && !spinStarted) {
            startSpin();
          } else if (p < 0.48 && spinStarted) {
            stopSpin();
          }
          if (p > 0.55 && spinStarted) {
    ...
    tl.to(awards, { autoAlpha: 0, duration: 0.12, ease: 'power1.in' }, 0.25);

    // PHASE 3: Footer appears (0.40 → 0.65)
    tl.to(footer, { autoAlpha: 1, duration: 0.06 }, 0.40);
    ...
    // PHASE 4: Hold for spin viewing (0.65 → 1.0)
